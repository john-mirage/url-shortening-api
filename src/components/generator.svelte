<script lang="ts">
  ...
  async function handleClipboard(linkIndex: number) {
    if (!links[linkIndex].copied) {
      let copiedLink = await navigator.clipboard.writeText(links[linkIndex].shortUrl).then(() => {
        return {
          baseUrl: links[linkIndex].baseUrl,
          shortUrl: links[linkIndex].shortUrl,
          copied: true
        }
      }, () => {
